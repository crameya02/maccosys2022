...
Public Class frmResponsibilityCenter
    Private Sub loadResponsibilityCenter()
        Try
            ...
            With RCDGV
                .DataSource = Nothing
                Dim str As String = String.Format("SELECT  RCID as ID, RCNumber as [Office Code], Office FROM ResponsibilityCenter  order by RCID asc")
    ...
    Private Sub txtSearchRC_TextChanged(sender As Object, e As EventArgs) Handles txtSearchRC.TextChanged
        Try
            ...
            With RCDGV
                ...
                Dim str As String = String.Format("SELECT  RCID as ID, RCNumber as [Office Code], Office  FROM ResponsibilityCenter  where RCNumber like '{0}%'  order by RCID asc", txtSearchRC.Text)
    ...
    End Sub
    Private Sub RCDGV_MouseClick(sender As Object, e As MouseEventArgs) Handles RCDGV.MouseClick
        Try
            If RCDGV.Rows.Count > 0 Then
                frmOBRE.OBREDGV.SelectedCells(0).Value = RCDGV.SelectedCells(1).Value.ToString
                Me.Close()
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
